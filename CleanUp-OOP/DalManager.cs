...
namespace CleanUp_OOP
{
    class DalManager
    {
        ...
        {
            List<CPUPower> cPUUsagesList = new List<CPUPower>();
            ...
            foreach (ManagementObject cpuUsage in searcher.Get())
            {
                ...
                CPUPower cpuUsageObj = new CPUPower(precentProcessorTime, usage);
        ...
        {
            List<OSSystem> osList = new List<OSSystem>();
            ...
            foreach (ManagementObject result in osResults)
            {
                ...
                OSSystem osObject = new OSSystem(user, organization, os);
                osList.Add(osObject);
            }
            return osList;
        }
        ...
        public static List<BootDrive> GetBootDrives()
        {
            ...
            List<BootDrive> bootDriveList = new List<BootDrive>();
            ...
            foreach (ManagementObject bootDrive in bootCollection)
            {
                string device = bootDrive["BootDevice"].ToString();

                BootDrive bootDeviceObj = new BootDrive(device);
                bootDriveList.Add(bootDeviceObj);
            }
            return bootDriveList;
        }
